...
useEffect(() => {
  ...
  const simpleCharName = charName.filter((text)=>{
    let charNameSplitted: string[] | undefined = text.textContent?.split(" ")
    if(charNameSplitted?.length > 3){
      
    }
    console.log(charNameSplitted);
    
  })
  
  // charName.forEach((name: any)=>{
  //   let nameContent = name.textContent?.split(" ")
  //   if(nameContent.length >= 3){
  //     nameContent.slice(0, 2)
  //     let splittedName = nameContent.join(" ")
  //     console.log("Nome junto:" + splittedName);
  //   }
